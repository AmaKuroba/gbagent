...
/// Reward system combining screen novelty and optional RAM-based scanners.
...
impl RewardSystem {
    ...
    pub fn compute(&mut self, emulator: &mut Emulator, frame: &[u8]) -> f64 {
        ...
        // 1. Screen novelty — pixel difference from previous frame
        if let Some(prev) = &self.prev_frame {
            if frame.len() == prev.len() {
                ...
                // Normalize by resolution * max diff * channels
                ...
                if avg_diff > 5.0 {
                    ...
                    // Anti-grind: small penalty for stale screens
                    self.stale_steps += 1;
    ...
    /// Return the last reward breakdown for debugging.
