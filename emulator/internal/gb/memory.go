...
// readOAM handles OAM reads with DMG bus conflict corruption.
func (m *MemoryBus) readOAM(addr uint16) byte {
...
func (m *MemoryBus) readIO(addr uint16) byte {
...
// writeOAM handles OAM writes with DMG bus conflict corruption.
func (m *MemoryBus) writeOAM(addr uint16, val byte) {
	if m.ppu != nil && m.ppu.GetMode() == ppuModeOAM {
		row := m.ppu.GetOAMRow()
		if row > 0 {
...
func (m *MemoryBus) writeIO(addr uint16, val byte) {
	if addr == 0xFF00 && m.joypad != nil {
